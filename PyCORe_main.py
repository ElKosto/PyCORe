...
#        return a*np.exp(1j*np.random.uniform(-1,1,self.N_points)*np.pi)
        ...
        def Propagate_SAM(self, simulation_parameters, Pump, Seed=[0]):
            ...
            def RHS(Time, A):
                ...
                dAdT[(self.N_CROW-1)*self.N_points+ind_modes] += 1j*self.J[:,self.N_CROW-2]*2/self.kappa_0 *np.exp(1j*ind_modes*np.pi)*A[((self.N_CROW-2))*self.N_points+ind_modes]+1j*np.fft.fft(A_dir[(self.N_CROW-1)*self.N_points+ind_modes]*np.abs(A_dir[(self.N_CROW-1)*self.N_points+ind_modes])**2)/self.N_points
                for ii in range(1,self.N_CROW-2):
